...
// SPEC-UX-012: 아코디언 구조 + 링크 수 배지 + 헤더 토글 + 칩 레이아웃 테스트
...
// SPEC-UX-012: favoritesExpandStore 모킹 — isExpanded / toggleExpand 제어
let mockIsExpanded = true
const mockToggleExpand = vi.fn()
vi.mock('../../stores/favoritesExpandStore', () => ({
  useFavoritesExpandStore: (selector: (s: { isExpanded: (id: string) => boolean; toggleExpand: (id: string) => void }) => unknown) => {
    const state = {
      isExpanded: (_id: string) => mockIsExpanded,
...
async function renderCard(category = mockCategory, isEditing = false, isExpanded = true) {
...
describe('BookmarkCard (SPEC-UX-006 + SPEC-UX-007)', () => {
  ...
  it('링크 목록이 모두 렌더링된다 (펼침 상태)', async () => {
    await renderCard()
  ...
  it('그룹 핸들이 카테고리 헤더의 첫 번째 자식이어야 한다 (REQ-UX-009-004)', async () => {
    await renderCard(mockCategory, true)
    const handle = document.querySelector('[data-group-handle]')
    // 핸들의 부모(래퍼 div)의 부모(카테고리 헤더)에서 첫 자식이 핸들 래퍼이어야 함
    const handleWrapper = handle?.parentElement
    const categoryHeader = handleWrapper?.parentElement
    expect(categoryHeader?.firstElementChild).toBe(handleWrapper)
  })

  // ─── SPEC-UX-012: 아코디언 구조 테스트 ─────────────────────────────────────

  // AC-007: 링크 수 배지 "(N)" 표시 (REQ-UX-012-005)
  it('헤더에 링크 수 배지 "(2)"가 표시되어야 한다 (AC-007)', async () => {
    await renderCard()
    expect(screen.getByText('(2)')).toBeInTheDocument()
  })

  // AC-007: 링크 0개 카테고리 배지 "(0)" 표시
  it('링크 0개 카테고리에서 헤더에 "(0)" 배지가 표시되어야 한다 (EDGE-003)', async () => {
    const emptyCategory: Category = { id: 'empty-1', name: '빈 카테고리', icon: '📂', links: [] }
    await renderCard(emptyCategory)
    expect(screen.getByText('(0)')).toBeInTheDocument()
  })

  // AC-012: 헤더 ARIA disclosure 속성 — aria-expanded, aria-controls
  it('헤더 토글 요소에 aria-expanded가 있어야 한다 (AC-012)', async () => {
    await renderCard(mockCategory, false, true)
    const header = document.querySelector('[data-category-handle]')
    expect(header).toHaveAttribute('aria-expanded', 'true')
  })

  it('접힘 상태에서 aria-expanded가 false이어야 한다 (AC-012)', async () => {
    await renderCard(mockCategory, false, false)
    const header = document.querySelector('[data-category-handle]')
    expect(header).toHaveAttribute('aria-expanded', 'false')
  })

  it('헤더에 aria-controls 속성이 있어야 한다 (AC-012)', async () => {
    await renderCard(mockCategory, false, true)
    const header = document.querySelector('[data-category-handle]')
    expect(header).toHaveAttribute('aria-controls', `bookmark-panel-${mockCategory.id}`)
  })

  // AC-009: 헤더 클릭 시 toggleExpand 호출 (REQ-UX-012-006)
  it('헤더 본문 클릭 시 toggleExpand가 호출되어야 한다 (AC-009)', async () => {
    await renderCard(mockCategory, false, true)
    const nameSpan = screen.getByText('Work')
    fireEvent.click(nameSpan)
    expect(mockToggleExpand).toHaveBeenCalledWith(mockCategory.id)
  })

  // AC-010: ⚙️ 버튼 클릭 시 toggleExpand 미발생 (stopPropagation)
  it('⚙️ 버튼 클릭 시 toggleExpand가 호출되어서는 안 된다 (AC-010)', async () => {
    const { onEdit } = await renderCard(mockCategory, true, true)
    const settingsBtn = screen.getByTestId('bookmark-edit-btn')
    fireEvent.click(settingsBtn)
  ...
  // AC-013: 키보드 Enter 토글 (REQ-UX-012-007)
  it('헤더에서 Enter 키 입력 시 toggleExpand가 호출되어야 한다 (AC-013)', async () => {
    await renderCard(mockCategory, false, true)
    const header = document.querySelector('[data-category-handle]') as HTMLElement
    fireEvent.keyDown(header, { key: 'Enter' })
    expect(mockToggleExpand).toHaveBeenCalledWith(mockCategory.id)
  })

  // AC-013: 키보드 Space 토글 (REQ-UX-012-007)
  it('헤더에서 Space 키 입력 시 toggleExpand가 호출되어야 한다 (AC-013)', async () => {
    await renderCard(mockCategory, false, true)
    const header = document.querySelector('[data-category-handle]') as HTMLElement
  ...
  // AC-014: 펼친 카테고리 = flex-wrap 컨테이너 렌더 (REQ-UX-012-008)
  it('펼침 상태에서 칩 컨테이너가 flex flex-wrap으로 렌더되어야 한다 (AC-014)', async () => {
    await renderCard(mockCategory, false, true)
    const panel = document.getElementById(`bookmark-panel-${mockCategory.id}`)
    expect(panel).toBeInTheDocument()
    expect(panel).toHaveStyle({ display: 'flex', flexWrap: 'wrap' })
  })

  // AC-015: 접힌 카테고리 = 칩 미표시 (REQ-UX-012-008)
  it('접힘 상태에서 링크 칩이 렌더되지 않아야 한다 (AC-015)', async () => {
    await renderCard(mockCategory, false, false)
    // 패널이 없거나 링크가 DOM에 없어야 함
    const panel = document.getElementById(`bookmark-panel-${mockCategory.id}`)
  ...
  // AC-029 변형: 링크가 없는 카테고리에 empty placeholder가 표시된다 (SPEC-UX-012 D6: "북마크가 없습니다")
  it('링크가 없는 카테고리(펼침)에 data-empty-placeholder 요소가 표시된다', async () => {
    const emptyCategory: Category = { id: 'empty-1', name: '빈 카테고리', icon: '📂', links: [] }
    await renderCard(emptyCategory, false, true)
    expect(document.querySelector('[data-empty-placeholder]')).toBeInTheDocument()
    // SPEC-UX-012 D6: cross-group 비목표 반영 — "북마크가 없습니다"로 단순화
    expect(screen.getByText('북마크가 없습니다')).toBeInTheDocument()
  })

  // AC-030: 링크가 있는 카테고리에 empty placeholder가 없다
  it('링크가 있는 카테고리에는 data-empty-placeholder 요소가 없다', async () => {
    await renderCard(mockCategory, false, true)
    expect(document.querySelector('[data-empty-placeholder]')).not.toBeInTheDocument()
  })

  // 접힘 상태 = placeholder 미표시
  it('접힘 상태에서는 placeholder가 표시되지 않아야 한다 (EDGE-003)', async () => {
    const emptyCategory: Category = { id: 'empty-1', name: '빈 카테고리', icon: '📂', links: [] }
    await renderCard(emptyCategory, false, false)
    expect(document.querySelector('[data-empty-placeholder]')).not.toBeInTheDocument()
  })

  // ─── SPEC-UX-012: 편집 모드 시각 단서 보존 (REQ-UX-012-018) ────────────────

  // AC-027: 편집 모드 ON 시 accent border 보존
  it('편집 모드 ON 시 accent border가 적용되어야 한다 (AC-027)', async () => {
    await renderCard(mockCategory, true, true)
    const card = document.querySelector('[data-category-handle]')?.parentElement
    expect(card).toHaveStyle({ border: '1px solid var(--accent)' })
  })
})
